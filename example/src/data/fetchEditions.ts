...
const fetchEditions = async (paths: string[]): Promise<GridButton[]> => {
    const editions = await Promise.all(paths.map(async path => {
      ...
      return {
        ...
        image: null,
        // data.data.section.image.cover[0]?.url.canonical,
      }
    }))
